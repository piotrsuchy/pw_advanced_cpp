...
LevelRenderer::LevelRenderer()
{
    ...
    {
        std::cout << "Power pellet texture loaded: " << powerPelletTexture.getSize().x << "x" << powerPelletTexture.getSize().y << "\n";
    }
...
void LevelRenderer::draw(sf::RenderWindow &window, const LevelManager &level)
{
    ...
    std::cout << "Recalculated offset: window(" << windowWidth << "," << windowHeight
              << ") - level(" << totalWidth << "," << totalHeight
              << ") / 2 = (" << offsetX << "," << offsetY << ")" << std::endl;
